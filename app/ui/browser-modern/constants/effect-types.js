/*
Copyright 2016 Mozilla

Licensed under the Apache License, Version 2.0 (the "License"); you may not use
this file except in compliance with the License. You may obtain a copy of the
License at http://www.apache.org/licenses/LICENSE-2.0
Unless required by applicable law or agreed to in writing, software distributed
under the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR
CONDITIONS OF ANY KIND, either express or implied. See the License for the
specific language governing permissions and limitations under the License.
*/

// Session effects

export const SET_SESSION_KEY = 'SET_SESSION_KEY_EFFECT';
export const DELETE_SESSION_KEY = 'DELETE_SESSION_KEY_EFFECT';

export const SESSION_RESTORE_BROWSER_WINDOW_APP_STATE = 'SESSION_RESTORE_BROWSER_WINDOW_APP_STATE_EFFECT';
export const START_SAVING_BROWSER_WINDOW_APP_STATE = 'START_SAVING_BROWSER_WINDOW_APP_STATE_EFFECT';
export const STOP_SAVING_BROWSER_WINDOW_APP_STATE = 'STOP_SAVING_BROWSER_WINDOW_APP_STATE_EFFECT';

export const NOTIFY_BROWSER_WINDOW_APP_STATE_SAVED = 'NOTIFY_BROWSER_WINDOW_APP_STATE_SAVED_EFFECT';
export const NOTIFY_BROWSER_WINDOW_APP_STATE_DESTROYED = 'NOTIFY_BROWSER_WINDOW_APP_STATE_DESTROYED_EFFECT';
export const NOTIFY_BROWSER_WINDOW_APP_STATE_WATCHED = 'NOTIFY_BROWSER_WINDOW_APP_STATE_WATCHED_EFFECT';
export const NOTIFY_BROWSER_WINDOW_APP_STATE_NOT_WATCHED = 'NOTIFY_BROWSER_WINDOW_APP_STATE_NOT_WATCHED_EFFECT';

// Developer effects

export const PERF_RECORD_START = 'PERF_RECORD_START_EFFECT';
export const PERF_RECORD_STOP = 'PERF_RECORD_STOP_EFFECT';

// External effects

export const MINIMIZE_WINDOW = 'MINIMIZE_WINDOW_EFFECT';
export const MAXIMIZE_WINDOW = 'MAXIMIZE_WINDOW_EFFECT';
export const CLOSE_WINDOW = 'CLOSE_WINDOW_EFFECT';
export const RELOAD_WINDOW = 'RELOAD_WINDOW_EFFECT';
export const OPEN_APP_MENU = 'OPEN_APP_MENU_EFFECT';

// Clipboard effects

export const COPY_TEXT_TO_CLIPBOARD = 'COPY_TEXT_TO_CLIPBOARD_EFFECT';

// Profile effects

export const FETCH_LOCATION_AUTOCOMPLETIONS = 'FETCH_LOCATION_AUTOCOMPLETIONS_EFFECT';
export const SET_REMOTE_BOOKMARK_STATE = 'SET_REMOTE_BOOKMARK_STATE_EFFECT';
export const ADD_REMOTE_HISTORY = 'ADD_REMOTE_HISTORY_EFFECT';
export const ADD_REMOTE_CAPTURED_PAGE = 'ADD_REMOTE_CAPTURED_PAGE_EFFECT';

// Page effects

export const CREATE_PAGE_SESSION = 'CREATE_PAGE_SESSION_EFFECT';
export const DESTROY_PAGE_SESSION = 'DESTROY_PAGE_SESSION_EFFECT';
export const BULK_CREATE_STANDALONE_PAGE_SESSIONS = 'BULK_CREATE_STANDALONE_PAGE_SESSIONS_EFFECT';
export const BULK_DESTROY_STANDALONE_PAGE_SESSIONS = 'BULK_DESTROY_STANDALONE_PAGE_SESSIONS_EFFECT';
export const NAVIGATE_PAGE_TO = 'NAVIGATE_PAGE_TO_EFFECT';
export const NAVIGATE_PAGE_BACK = 'NAVIGATE_PAGE_BACK_EFFECT';
export const NAVIGATE_PAGE_FORWARD = 'NAVIGATE_PAGE_FORWARD_EFFECT';
export const NAVIGATE_PAGE_REFRESH = 'NAVIGATE_PAGE_REFRESH_EFFECT';
export const NAVIGATE_PAGE_IN_HISTORY = 'NAVIGATE_PAGE_IN_HISTORY_EFFECT';
export const TOGGLE_DEVTOOLS = 'TOGGLE_DEVTOOLS_EFFECT';
export const PERFORM_DEVTOOLS_INSPECT_ELEMENT = 'PERFORM_DEVTOOLS_INSPECT_ELEMENT_EFFECT';
export const PERFORM_PAGE_DOWNLOAD = 'PERFORM_PAGE_DOWNLOAD_EFFECT';
export const PERFORM_PAGE_SEARCH = 'PERFORM_PAGE_SEARCH_EFFECT';
export const SET_PAGE_ZOOM_LEVEL = 'SET_PAGE_ZOOM_LEVEL_EFFECT';
export const CAPTURE_PAGE = 'CAPTURE_PAGE_EFFECT';
export const PARSE_PAGE_META_DATA = 'PARSE_PAGE_META_DATA_EFFECT';
export const GET_CERTIFICATE_ERROR = 'GET_CERTIFICATE_ERROR_EFFECT';
export const DISPLAY_WEBVIEW_CONTEXT_MENU = 'DISPLAY_WEBVIEW_CONTEXT_MENU_EFFECT';
export const PIN_TAB = 'PIN_TAB_EFFECT';
export const UNPIN_TAB = 'UNPIN_TAB_EFFECT';
export const FORK_PAGE_BACK = 'FORK_PAGE_BACK';
export const FORK_PAGE_FORWARD = 'FORK_PAGE_FORWARD';

// UI effects

export const SET_APPLICATION_ACTIVE_ELEMENT = 'SET_APPLICATION_ACTIVE_ELEMENT_EFFECT';
export const FOCUS_URL_BAR = 'FOCUS_URL_BAR_EFFECT';
export const SET_URL_BAR_VALUE = 'SET_URL_BAR_VALUE_EFFECT';
export const SHOW_DOWNLOAD_NOTIFICATION = 'SHOW_DOWNLOAD_NOTIFICATION_EFFECT';
export const CONFIRM_DEFAULT_BROWSER = 'CONFIRM_DEFAULT_BROWSER_EFFECT';
